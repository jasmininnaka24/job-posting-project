...
df_src = spark.sql(f'''SELECT * FROM job_posting_catalog.silver.{silver_tables['processed_jobs']}''')
...
if spark.catalog.tableExists(f"job_posting_catalog.gold.{gold_tables['dim_jobs']}"):
    df_sink = spark.sql(f'''SELECT * FROM job_posting_catalog.gold.{gold_tables['dim_jobs']}''')
else:
    df_sink = spark.sql(f'''
                        SELECT 1 AS dim_job_key, *
                        FROM job_posting_catalog.silver.{silver_tables['processed_jobs']}
                        WHERE 1=0
                        ''')
...
df_filter_old = df_filter.filter(col('dim_job_key').isNotNull())
...
df_filter_new = df_filter.filter(col('dim_job_key').isNull())
...
if v_incremental_flag == '0':
    ...
    max_value = spark.sql(f'''SELECT MAX(dim_job_key) FROM job_posting_catalog.gold.{gold_tables['dim_jobs']}''')
...
df_filter_new = df_filter_new.withColumn('dim_job_key', max_value+monotonically_increasing_id())
...
table_name = gold_tables['dim_jobs']
